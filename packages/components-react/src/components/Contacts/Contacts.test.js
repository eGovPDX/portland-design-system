...
describe("Contacts", () => {
  it("renders all contact information when provided", () => {
    ...
    expect(
      screen.getByRole("link", { name: /contact councilor via email/i }),
    ...
    expect(socialLinks[0]).toHaveAttribute(
      "href",
      "https://facebook.com/portlandgov",
    );
    ...
    expect(socialLinks[2]).toHaveAttribute(
      "href",
      "https://instagram.com/portlandgov",
    );

    // Check office information
    const addressLink = screen.getByRole("link", {
      name: /SW 123 Normal Street.*Portland, OR 97204/i,
    });
    expect(addressLink).toBeInTheDocument();
    expect(addressLink).toHaveAttribute(
      "href",
      "https://maps.google.com/?q=SW 123 Normal Street Portland, OR 97204",
    );
    ...
    const hoursValues = screen.getAllByText((content, element) => {
      return (
        element.textContent.includes("Monday - Friday") &&
