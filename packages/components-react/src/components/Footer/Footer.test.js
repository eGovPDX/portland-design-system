...
describe("Footer", () => {
  ...
  test("renders accessibility and translation information", () => {
    ...
    expect(
      screen.getByText(/The City of Portland ensures meaningful access/i),
    ).toBeInTheDocument();
    ...
    expect(
      screen.getByText(/Traducción e Interpretación/i),
    ).toBeInTheDocument();
  });
  ...
  test("renders navigation sections", () => {
    ...
    expect(
      screen.getByRole("link", { name: "311@portlandoregon.gov" }),
    ).toHaveAttribute("href", "mailto:311@portlandoregon.gov");
    expect(
      screen.getByRole("link", { name: "ADA accommodation" }),
    ).toHaveAttribute("href", "/ada-accommodation");
  });
  ...
  test("renders city information", () => {
    ...
    expect(screen.getByAltText("City of Portland Seal")).toHaveAttribute(
      "src",
      "/city-seal.png",
    );
